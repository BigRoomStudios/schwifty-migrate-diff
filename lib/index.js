...
exports.genMigrationFile = (server, options, next) => {
    ...
    Joi.assert(options, Schema.options, 'Bad options passed to schwifty-migration diffModels.');
    ...
    internals.diffTables(migrationGroups, (err) => {
...
internals.diffTables = (migrationGroups, cb) => {
    ...
    Items.serial(knexes, (knex, next) => {
        ...
        const getTablesCb = (dbTableNames) => {
            ...
            const joinTables = models.map((model) => {

                return internals.getJoinTableInfo(model);
            });

            const changes = internals.diffJson(dbTableNames, modelTableNames);

            console.log(changes);

            changes.create.forEach((tableToCreate) => {

                // getMGroupWithModel(tableToCreate).migrationsDir
            });

            changes.alter.forEach((tableToAlter) => {
