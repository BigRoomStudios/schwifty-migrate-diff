...
exports.genMigrationFile = (options, schwiftyMigrationNext) => {
    ...
    .reduce((collector, joinTable) => {
        ...
        if (!collector[tableName]) {
            ...
            const joiAsKnexSchema = joi2Knex(joiSchema);

            if (internals.isError(joiAsKnexSchema)) {
                joinTableErrors.push(joiAsKnexSchema);
            }
            else {
                collector[tableName] = {
                    joiSchema,
    ...
    if (joinTableErrors.length > 0) {
        return schwiftyMigrationNext(joinTableErrors);
    }

    const regularTableErrors = [];

    const regularTables = migrationGroup.models.reduce((collector, model) => {

        const joi2KnexSchema = joi2Knex(model.joiSchema);

        if (internals.isError(joi2KnexSchema)) {
            regularTableErrors.push(joi2KnexSchema);
        }
        else {
    ...
    if (regularTableErrors.length > 0) {
        return schwiftyMigrationNext(regularTableErrors);
    }
    ...
    internals.diffTables(migrationGroup, joinTables, allTables, (_tableErr, tableDeltas) => {
        ...
        internals.diffColumns(migrationGroup, (columnsErr, columnDeltas) => {

            if (columnsErr) {
                return schwiftyMigrationNext(columnsErr);
            }
...
internals.diffTables = (migrationGroup, joinTables, allTables, cb) => {
    ...
    */
    ...
    () => {
        ...
        // there can't be any actual errors
...
internals.diffColumns = (migrationGroup, cb) => {
    ...
    Items.serial(models, (model, modelNext) => {
        ...
        if (internals.isError(modelKnexSchema)) {
            return cb(modelKnexSchema);
        }
        ...
        .then((dbSchema) => {
            ...
            const db2ColumnCompilerErrors = [];

            const db2ColumnCompilerSchema = dbColsToConvert.reduce((collector, columnName) => {

                const currentColumn = dbSchema[columnName];
                const db2ColumnCompilerConversion = db2ColumnCompiler(currentColumn);

                if (internals.isError(db2ColumnCompilerConversion)) {
                    db2ColumnCompilerErrors.push(db2ColumnCompilerConversion);
                }
                else {
            ...
            if (db2ColumnCompilerErrors.length > 0) {
                return cb(db2ColumnCompilerErrors);
            }
                ...
                Object.keys(delta).forEach((key) => {
                    ...
                    switch (internals.getJsonDiffpatchChangeType(delta[key])) {
                        ...
                        case 'alter':
                            ...
                            const db2KnexKey0 = db2Knex(delta[key][0]);
                            const db2KnexKey1 = db2Knex(delta[key][1]);

                            if (internals.isError(db2KnexKey0) || internals.isError(db2KnexKey1)) {
                                if (internals.isError(db2KnexKey0)) {
                                    db2KnexErrs.push(db2KnexKey0);
                                }
                                if (internals.isError(db2KnexKey1)) {
                                    db2KnexErrs.push(db2KnexKey1);
                                }
                            }
                            else {
                        ...
                        case 'drop':
                            ...
                            const db2KnexKey = db2Knex(dbSchema[key]);

                            if (internals.isError(db2KnexKey)) {
                                db2KnexErrs.push(db2KnexKey);
                            }
                            else {
                                modelDeltaObject.drop.push({
                                    columnName: key,
    ...
    (err) => {
        ...
        return cb(err, knexColumnDeltaObject);
    });
};
...
internals.isError = (err) => {

    if (Array.isArray(err)) {
        return err.every((arrErr) => arrErr instanceof Error);
    }
