                ...
                internals.diffTables(migrationGroup, joinTables, allTables, userOptions.mode, (err, tableDeltas) => {
                    ...
                    internals.diffColumns(migrationGroup, joinTables, userOptions.mode, (diffColsErr, columnDeltas) => {
                        ...
                        if (!!internals.skippedCols.length) {
                            skippedColsMsg = `Skipped unsupported columns:${Os.EOL}${internals.skippedCols.join(Os.EOL)}`;
                        }
        ...
        Promise.resolve(knex(tableName).select(knex.raw('1')))
            .asCallback((err) => {
                ...
                if (err) {
                    ...
                    if (err.message !== `select 1 from "${tableName}" - relation "${tableName}" does not exist`) {
...
internals.diffColumns = (migrationGroup, joinTables, mode, cb) => {
    ...
    Items.serial(models.concat(joinTablesAsModels), (model, next) => {
        ...
        // If a join table has bad columns, it never gets added to the joinTables
        // object. So each item in this list has already cleared this mapping check
        ...
        internals.getColumnsForTable(knex, modelName, (err, dbSchema) => {
            ...
            const db2ColumnCompilerSchema = dbColsToConvert.reduce((collector, columnName) => {
                ...
                const [err, db2ColumnCompilerConversion] = Mappings.convertFuncs.db2ColumnCompiler(currentColumn, ` for column "${columnName}"`);
            ...
            const normalizedDb2ColumnCompilerSchema = Object.keys(db2ColumnCompilerSchema).reduce((collector, keyName) => {
                ...
                if (!currentType) {
                    console.log('db2ColumnCompilerSchema', db2ColumnCompilerSchema);
                }
            ...
            if (reducedDb2CcErrors) {
                return next(reducedDb2CcErrors);
            }
            ...
            if (reducedKnex2CcErrors) {
                return next(reducedKnex2CcErrors);
            }
        ...
        internals.getColumnsForTable(knex, tableName, (err, dbSchema) => {
            ...
            const dbColsToConvert = Object.keys(dbSchema)
                .filter((colName) => {
                    ...
                    const [err, val] = Mappings.convertFuncs.getAliasVal(colType);
                    if (!!err) {
                        internals.skippedCols.push(`colName: ${colName}, colType: ${colType}, model: ${tableName}`);
                        return false;
                    }
                    return true;
                });
