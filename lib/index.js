...
const Db2KnexColumnCompilerTypes = require('./db2KnexColumnCompilerTypes');
    ...
    .reduce((collector, joinTable) => {
        ...
        if (!collector[joinTable.tableName]) {
            ...
            collector[joinTable.tableName].knexSchema = Object.assign(
                ...
                Joi2KnexSchema.convert(joinTable.joiSchema)
            );
        }
...
internals.diffColumns = (migrationGroups, joinTables, allTables, cb) => {
    ...
    Items.parallel(knexes, (knex, next) => {
        ...
        Items.parallel(Object.keys(models), (modelName, next) => {
            ...
            // This func spits out MySQL lookin column types. dunno why!
            // So I'll have a converter from other db types to MySQL for comparing afterwards
            const modelKnex2DbSchema = Object.keys(modelKnexSchema).reduce((collector, columnName) => {
                ...
                console.log('tmpColumnCompiler.getColumnType()', tmpColumnCompiler.getColumnType());
                console.log('tmpColumnCompiler.getModifiers()', tmpColumnCompiler.getModifiers());
            ...
            .then((dbSchema) => {
                ...
                const fixedDbSchema = Object.keys(dbSchema)
                .reduce((collector, columnName) => {

                    const currentColumn = dbSchema[columnName]
                    collector[columnName] = Db2KnexColumnCompilerTypes.convert(dbSchema[columnName]);
                    return collector;
                }, {});

                console.log('dbSchema', dbSchema);
                console.log('fixedDbSchema', fixedDbSchema);

                const delta = JsonDiffpatch.diff(fixedDbSchema, modelKnex2DbSchema);
                    ...
                    Object.keys(delta).forEach((key) => {
                        ...
                        switch (internals.getJsonDiffpatchChangeType(delta[key])) {
                            ...
                            case 'create':
                                ...
                                break;
                            case 'alter':
                                modelDeltaObject.alter[key] = [Db2KnexColumnCompilerTypes.convert(delta[key][0]), Db2KnexColumnCompilerTypes.convert(delta[key][1])];
                                break;
                            case 'drop':
                                modelDeltaObject.drop.push({
                                    columnName: key,
                                    columnType: Db2KnexColumnCompilerTypes.convert(dbSchema[key])
                                });
                                break;
                        }
                    });
