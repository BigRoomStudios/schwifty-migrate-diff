            ...
            (err) => {
                ...
                const joinTables = joinTableCollector.reduce((collector, joinTable) => {
                    ...
                    if (!collector[tableName]) {
                        ...
                        const [ignoreErr, joiAsKnexSchema] = Mappings.convertFuncs.joi2Knex(collector[tableName].joiSchema, ` in model "${tableName}"`);
                ...
                internals.diffTables(migrationGroup, joinTables, allTables, userOptions.mode, (err, tableDeltas) => {
                    ...
                    internals.diffColumns(migrationGroup, joinTables, userOptions.mode, (diffColsErr, columnDeltas) => {
                        ...
                        // Need this message for both cases where no migration is needed
                        // (because unsupported columns get skipped)
                        // and on a successful migration file generation where other
                        // columns are changed or tables added, etc.
                        let skippedColsMsg = '';
                        if (!!internals.skippedCols.length) {
                            const dedupedSkippedCols = internals.skippedCols.filter((skippedInfo, i) => {

                                return internals.skippedCols.indexOf(skippedInfo) === i;
                            });
                            skippedColsMsg = `Skipped unsupported columns:${Os.EOL}${dedupedSkippedCols.join(Os.EOL)}`;
                        }

                        if (createdTables.length === 0 &&
                            columnDeltas.changes.length === 0) {

                            const noMigrationMsg = !!skippedColsMsg ? `${skippedColsMsg}${Os.EOL}No migration needed` : 'No migration needed';
                            return cb(null, noMigrationMsg);
                        }
                        ...
                        migrationGroup.knex.migrate.make(userOptions.migrationName || 'schwifty-migration', {
                            ...
                            .asCallback((err) => {
                                ...
                                const successMsg = !!skippedColsMsg ? `${skippedColsMsg}${Os.EOL}new migration: ${justCreatedMigrationPath}` : justCreatedMigrationPath;
...
internals.diffColumns = (migrationGroup, joinTables, mode, cb) => {
    ...
    Items.serial(allModels, (model, next) => {
        ...
        const [ignoreErr, modelKnexSchema] = Mappings.convertFuncs.joi2Knex(model.joiSchema);
        ...
        internals.getColumnsForTable(knex, modelName, (err, dbSchema) => {
            ...
            const dbColsToConvert = Object.keys(dbSchema)
                .filter((colName) => {
                    ...
                    const [err, val] = Mappings.convertFuncs.getAliasVal(colType);
                    if (!!err) {
            ...
            const db2ColumnCompilerSchema = dbColsToConvert.reduce((collector, columnName) => {
                ...
                /* eslint-disable no-shadow */
                const [ignoreErr, db2ColumnCompilerConversion] = Mappings.convertFuncs.db2ColumnCompiler(currentColumn, ` for column "${columnName}"`);
            ...
            const normalizedDb2ColumnCompilerSchema = Object.keys(db2ColumnCompilerSchema).reduce((collector, keyName) => {
                ...
                /* eslint-disable no-shadow */
                const [ignoreErr, aliasVal] = Mappings.convertFuncs.getAliasVal(currentType);
                ...
                Object.keys(delta).forEach((key) => {
                    ...
                    switch (internals.getJsonDiffpatchChangeType(delta[key])) {
                        ...
                        case 'alter':
                            ...
                            const [ignoreErr0, db2KnexKey0] = Mappings.convertFuncs.db2Knex(delta[key][0]);
                            const [ignoreErr1, db2KnexKey1] = Mappings.convertFuncs.db2Knex(delta[key][1]);
                        ...
                        case 'drop':
                            ...
                            const [ignoreErr, db2KnexKey] = Mappings.convertFuncs.db2Knex(dbSchema[key]);
    ...
    Items.serial(Object.keys(model.relationMappings), (relationName, next) => {
        ...
        internals.getColumnsForTable(knex, tableName, (err, dbSchema) => {
            ...
            const db2JoiTypes = dbColsToConvert.reduce((collector, columnName) => {
                ...
                // produced here
                const [ignoreErr, knexType] = Mappings.convertFuncs.db2Knex(currentColumn);

                // It's mapping.js's job to keep the left side of knexJoiMap
                // in parity with the right side of columnCompilerKnexMap
                // No errors should be produced
                const [ignoreErrJoi, db2Joi] = Mappings.convertFuncs.knex2JoiType(knexType);
