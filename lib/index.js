...
exports.diffModels = (server, options, next) => {
    ...
    migrationGroup.models.forEach((model) => {
        ...
        .then((dbSchema) => {
            ...
            const delta = JsonDiffpatch.diff(modelKnex2DbSchema, dbSchema);
            console.log(delta);

            if (delta) {
                Object.keys(delta).forEach((key) => {

                    console.log(modelKnex2DbSchema[key]);
                });
            }
...
exports.diffDbTables = (server, options, next) => {

    Joi.assert(options, Schema.options, 'Bad options passed to schwifty-migration diffTables.');
