...
exports.genMigrationFile = (options, schwiftyMigrationNext) => {
    ...
    internals.migrationGroup = migrationGroup;
    ...
    internals.diffTables(joinTables, allTables, (err, tableDeltas) => {

        if (err) {
            return schwiftyMigrationNext(err);
        }

        internals.diffColumns((err, columnDeltas) => {
...
internals.diffTables = (joinTables, allTables, cb) => {

    // Diff the tables in the db with our models and joinTables

    const { knex, migrationsDir } = internals.migrationGroup;
...
internals.diffColumns = (cb) => {
    ...
    const { knex, models, migrationsDir } = internals.migrationGroup;

    const knexColumnDeltaObject = {
        changes: []
    };

    Items.parallel(models, (model, modelNext) => {
        ...
        .catch((err) => {
            ...
            return internals.schwiftyMigrationNext(err);
        });
    },
    (err) => {
        ...
        return cb(null, knexColumnDeltaObject);
    });
};
