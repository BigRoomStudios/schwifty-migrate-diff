...
// Below is taken straignt from Knex
...
exports.diffModelsWithDb = (server, options, next) => {

    Joi.assert(options, Schema.plugin, 'Bad options passed to schwifty-migration.');

    // const userOptions = Hoek.shallow(options);
    const userOptions = options;
    const migrationGroup = userOptions.migrationGroup;

    // Get model db schemas
    // const tableSchemas = {};

    // console.log(clientName);
    // console.log(require(`knex/src/dialects/${aliases[clientName] || clientName}/index.js`));

    migrationGroup.models.forEach((model) => {

        const modelKnexSchema = Joi2KnexSchema.convert(model.joiSchema);
        console.log(modelKnexSchema);

        // To diff our derived knex schema with what's in the db,
        // let's compile this knex schema using the same methods knex does under the hood.
        // columnCompiler contains the map from knex types to dialect column types

        const columnCompiler = migrationGroup.knex.client.columnCompiler;
        console.log(columnCompiler['integer']);
        return;
        const modelKnex2DbSchema = Object.keys(modelKnexSchema).reduce((collector, columnName) => {

            // console.log(columnCompiler[modelKnexSchema[columnName]]);

            collector[columnName] = columnCompiler[modelKnexSchema[columnName]];
            return collector;
        }, {});

        // console.log(modelKnex2DbSchema);
        // return;

        // const compiledKnexColumns = Object.keys(modelKnexSchema).reduce((collector, ) => {
        ...
        .then((dbSchema) => {
            ...
            Object.keys(dbSchema).forEach((columnName) => {
                ...
                console.log(migrationGroup.knex.client.columnCompiler[columnSchema.type]);
            });

            // console.log(dbSchema);

            // const delta = JsonDiffpatch.diff(dbSchema, modelKnexSchema);
            // console.log(delta);
        });
...
internals.getJsonDiffpatchChangeType = (change) => {
    ...
    switch (change.length) {
        ...
        case 3:
            ...
            // Just a lil extra validation before dropping it
