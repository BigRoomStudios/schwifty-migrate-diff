...
exports.genMigrationFile = (options, next) => {
    ...
    internals.diffTables(migrationGroups, joinTables, allTables, (err, knexTableDeltas) => {
        ...
        internals.diffColumns(migrationGroups, joinTables, allTables, (err, knexColumnDeltas) => {
            ...
            // console.log(JSON.stringify(allTables, undefined, 4));

            // console.log(JSON.stringify(knexColumnDeltas, undefined, 4));
            const compiler = Handlebars.compile(String(Fs.readFileSync(__dirname + '/migrationFileTemplate.hbs')));

            Object.keys(deltasByMigrationsDir).forEach((migrationsDir) => {

                console.log(JSON.stringify({
                    migrations: deltasByMigrationsDir[migrationsDir],
                    allTables
                }, undefined, 4));
                ...
                console.log(migrationFileContents);
            });
            ////////////////////////
        });
    });
...
internals.diffColumns = (migrationGroups, joinTables, allTables, cb) => {
    ...
    Items.parallel(knexes, (knex, next) => {
        ...
        Items.parallel(Object.keys(models), (modelName, next) => {
            ...
            knex.table(modelName).columnInfo()
            .then((dbSchema) => {
                    ...
                    Object.keys(delta).forEach((key) => {
                        ...
                        switch (internals.getJsonDiffpatchChangeType(delta[key])) {
                            ...
                            case 'create':
                                ...
                                break;
                            case 'alter':
                                modelDeltaObject.alter[key] = delta[key][1];
                                break;
                            case 'drop':
                                modelDeltaObject.drop.push(key);
                                break;
                        }
                    });
                }
        ...
        Object.keys(delta).forEach((key) => {
            ...
            switch (internals.getJsonDiffpatchChangeType(delta[key])) {
                ...
                case 'create':
                    ...
                    break;
                case 'alter':
                    changesByKey.alter[key] = delta[key][1];
                    break;
                case 'drop':
