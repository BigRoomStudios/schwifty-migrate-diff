...
exports.register = (server, options, next) => {
    ...
    if (typeof options.migration !== 'undefined') {
        Hoek.assert(collector.migration === null, 'Schwifty-migration\'s migration options can only be specified once.');
        collector.migration = options.migration;
    }

    server.ext('onPreStart', internals.initModels(options), { after: 'schwifty' });
...
internals.initModels = (options) => {
    ...
    return (server, next) => {
        ...
        const migrationsDir = options.dir + '/migrations';
