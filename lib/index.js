...
const { convertFuncs: { joi2Knex, db2ColumnCompiler }, ignoreColumns } = require('./mappings');
        ...
        Items.parallel(Object.keys(models), (modelName, next) => {
            ...
            .then((dbSchema) => {
                ...
                const db2ColumnCompilerSchema = dbColsToConvert.reduce((collector, columnName) => {
                    ...
                    collector[columnName] = db2ColumnCompiler(dbSchema[columnName]);
                    return collector;
                }, {})
                    ...
                    Object.keys(delta).forEach((key) => {
                        ...
                        switch (internals.getJsonDiffpatchChangeType(delta[key])) {
                            ...
                            case 'drop':
                                modelDeltaObject.drop.push({
                                    columnName: key,
                                    columnType: db2ColumnCompiler(dbSchema[key])
                                });
                                break;
                        }
                    });
