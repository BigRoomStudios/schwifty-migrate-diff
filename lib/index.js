    ...
    if (model.relationMappings) {
        ...
        Object.keys(model.relationMappings).forEach((relationName) => {
            ...
            if (relation.join.through.modelClass) {
                ...
                const extraJoiSchema = relationExtra && Joi.object(relationExtra.reduce((collector, extra) => {
                ...
                let joiSchema = Joi.object(toFromColumns);

                if (extraJoiSchema) {
                    joiSchema = extraJoiSchema.keys(toFromColumns);
                }
