...
const convertFuncs = {
    ...
    joi2Knex: (joiSchema) => {
        ...
        Hoek.assert(badChildren.length === 0, `Joi Schema type(s) "${badChildren}" not supported.`);
    ...
    knex2JoiType: (knexType) => {
        ...
        Hoek.assert(typeof converted !== 'undefined', `knexJoiMap doesn't support knex type "${knexType}".`);
    ...
    db2ColumnCompiler: (dbType) => {
        ...
        Hoek.assert(typeof foundType !== 'undefined', `Database type "${dbType}" not supported.`);
    ...
    columnCompiler2Knex: (columnCompilerType) => {
        ...
        Hoek.assert(typeof foundType !== 'undefined', `Column Compiler type "${columnCompilerType}" not supported.`);
